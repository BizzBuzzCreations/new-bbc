...
function cloudinaryError(json, fallback) {
  ...
  // Cloudinary says "Invalid Signature" when the server's
  // CLOUDINARY_API_SECRET doesn't belong to CLOUDINARY_API_KEY — a server
  // configuration problem, not something the person uploading can fix.
  if (msg?.startsWith("Invalid Signature")) {
    return "Upload failed: the server's Cloudinary API secret does not match its API key. Please ask the developer to update CLOUDINARY_API_SECRET on the server.";
  }
  return msg ? `Upload failed: ${msg}` : fallback;
}
